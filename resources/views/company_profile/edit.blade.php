...
@section('content')
    <div class="container">
        <form action="{{ route('company_profile.update', $companyProfile->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="company_id">Select Category</label>
                <select name="category_id" id="category_id" class="form-control js-example-responsive" required
                    style="height: 45px; border: 1px solid #DCDFE8; -webkit-border-radius: 10px; border-radius: 10px;">
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}"
                            {{ $category->id == $companyProfile->category_id ? 'selected' : '' }}>{{ $category->category_name }}
                        </option>
                    @endforeach
                </select>
            </div>
